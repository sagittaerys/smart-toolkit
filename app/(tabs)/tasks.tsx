...
export default function TasksScreen() {
  ...
  function handleSave() {
    ...
    if (editingTask) {
      editTask(editingTask.id, titleInput.trim(), noteInput.trim() || undefined);
    } else {
        ...
        <ScrollView contentContainerStyle={s.list} showsVerticalScrollIndicator={false}>
          ...
          {filtered.map((task) => (
            <View key={task.id} style={s.taskCard}>
              <TouchableOpacity onPress={() => handleToggle(task.id)} style={s.checkbox}>
                <View style={[s.checkboxInner, task.completed && s.checkboxDone]}>
              ...
              <View style={s.taskContent}>
                ...
                {task.note ? (
                  <Text style={s.taskNote} numberOfLines={1}>{task.note}</Text>
                ) : null}
              </View>
              <View style={s.taskActions}>
                <TouchableOpacity onPress={() => openEdit(task)} style={s.actionBtn}>
                  <Text style={s.editText}>✎</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => handleDelete(task.id)} style={s.actionBtn}>
...
const s = StyleSheet.create({
  ...
  addBtnText: { color: Colors.inkPrimary, fontSize: FontSize.sm, fontWeight: "700" },
  filterBar: {
    flexDirection: "row", backgroundColor: Colors.surface,
    borderRadius: Radius.md, padding: 4,
    borderWidth: 1, borderColor: Colors.border,
  },
  filterBtn: { flex: 1, paddingVertical: Spacing.sm, borderRadius: Radius.sm, alignItems: "center" },
  filterBtnActive: { backgroundColor: Colors.brand },
  filterText: { color: Colors.inkMuted, fontSize: FontSize.xs, fontWeight: "600" },
  ...
  taskContent: { flex: 1 },
  taskTitle: { color: Colors.inkPrimary, fontSize: FontSize.md, fontWeight: "500" },
  ...
  modalTitle: { color: Colors.inkPrimary, fontSize: FontSize.lg, fontWeight: "700" },
  modalInput: {
